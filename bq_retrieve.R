...
process_a_subset <- function(df) {
  ...
  df_wide <- as_tibble(
    cbind(df[, !names(df) %in% c("attributes", "jattr")], jattr_tibble),
    .name_repair = "unique"
  )
  ...
  df_wide_in <- apply(keywords_x2, 1, function(x) {
    df_wide_ex %>%
      select(where(is.character)) %>%
      filter(
        if_any(everything(), ~ grepl(x[1], ., ignore.case = T)) &
          if_any(everything(), ~ grepl(x[2], ., ignore.case = T))
      )
  })
...
ggplot(output, aes(V1, V2)) +
  ...
  scale_fill_gradient(low = "white", high = "orange", name = "SRA runs") +
  labs(
    title = "Number of SRA runs given a combination of two keywords",
...
ggplot(output, aes(V1, V2)) +
  ...
  scale_fill_gradient(low = "white", high = "lightblue", name = "Size (Gb)") +
  labs(
    title = "Total size of SRA runs given a combination of two keywords",
    subtitle = "Diagonal represents a single keyword"
  ) +
  theme(
    axis.title = element_blank(),
...
calc_mbytes <- function(dataframe, ...) {
  ...
  for (keyword in keywords) {
    i <-
      ...
      filter(if_any(
        everything(),
        ~ grepl(paste0("\\b", keyword, "\\b"), ., ignore.case = T)
      ))
  }
...
wb_Fworker <-
  ...
  filter(
    (if_any(everything(), ~ grepl("\\bworker(s)?\\b", ., ignore.case = T))) |
      (if_any(everything(), ~ grepl("\\bfemale(s)?\\b", ., ignore.case = T)))
  )
...
wb <-
  ...
  filter(
    (if_any(
      everything(),
      ~ grepl("\\bwhole.?bod[yi](es)?\\b", ., ignore.case = T)
    ))
  )
...
output <- parLapply(cl, 1:10, process_a_subset(data_to_parse))
